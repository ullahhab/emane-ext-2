...
def gettime(string, index): #index is index at which 'Queue 2 has size' begins
    temphr = strfile[index - 55: index - 39]
    # print temphr
...
file = open("persist/1/var/log/mgen.log","r")

start = 0

strfile = file.read()

indlist = find_substring("PERIODIC [", strfile)  # Replace the substring with whichever queue size you require
...
for i in range(0,len(indlist)-3,3):
    currsize = 0
    for j in range(3):
        temp = strfile[indlist[i+j]+10: indlist[i+j]+24] # substring
        print strfile[indlist[i+j]-20: indlist[i+j]+24], temp
        temp1 = temp.split()   # list
        currsize += float(temp1[0])
